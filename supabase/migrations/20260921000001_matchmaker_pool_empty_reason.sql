-- MatchMaker pool: JSONB envelope with empty_reason diagnosis + distance_km per profile.

CREATE OR REPLACE FUNCTION public.matchmaker_haversine_km(
...
BEGIN
  ...
  IF v_has_location_dealbreaker THEN
    SELECT EXISTS (
      SELECT 1
      FROM public.profiles p
      INNER JOIN public.users u ON u.id = p.user_id
