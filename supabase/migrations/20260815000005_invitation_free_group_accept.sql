...
BEGIN
  IF COALESCE(p_plan.is_group_plan, false) THEN
    IF COALESCE(p_plan.is_paid, false) THEN
      _amount := public.resolve_join_request_slot_cents(p_plan);
    ELSIF p_plan.escrow_pattern = 'B' OR public.is_group_split_dynamic_plan(p_plan) THEN
      _amount := COALESCE(
    ...
    ELSE
      _amount := 0;
    END IF;
    RETURN GREATEST(0, COALESCE(_amount, 0));
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    IF COALESCE(p_plan.is_group_plan, false) AND NOT COALESCE(p_plan.is_paid, false) THEN
      RETURN 0;
    END IF;
    RAISE;
END;
...
GRANT EXECUTE ON FUNCTION public.resolve_invitation_accept_slot_cents(public.plans) TO authenticated, service_role;
...
BEGIN
  ...
  IF p_action = 'accept' THEN
    ...
    IF COALESCE(_plan.is_negotiable, true) THEN
      _slot_amount := public.resolve_invitation_accept_slot_cents(_plan);
      IF _slot_amount <= 0 AND COALESCE(_plan.is_paid, false) THEN
    ...
    ELSE
      ...
      IF COALESCE(_plan.is_group_plan, false)
        AND NOT COALESCE(_plan.is_paid, false)
        AND _slot_amount <= 0 THEN
